...
````json
{
    "signed_request": "https://<file direct download url>"
}
````

This method returns an `HTTP 200` response if the request is successful. The content under the key `signed_request` is a direct, authtenticated and secure link to download a file
found in the `filesDeliveredByManagers` array in the Project JSON. Please note the trailing bar at the end of this request.

### HTTP Request

`GET https://bureau.works/api/pub/v1/project/{id}/{serviceItemId}/{filename}/`
